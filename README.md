...
| LLMベースの要望分類 | 実音声認識（faster-whisper）・実姿勢推定/離床検知（MediaPipe）は実装済み。要望分類自体は依然としてキーワードマッチのルールベースで、LLMは使用していない | Phase 5以降 |
| 複数病棟対応・ロボットの自動割り当て | マルチロボット対応（`robot_id`パラメータ化、IDLE/BUSYステータス）は実装済み。「どの患者にどのロボットを割り当てるか」の自動化（`domain_service.pick_available_robot_id()`）と複数病棟（Ward単位での運用分離）は土台のみで未接続 | Phase 5 |
...
- 実音声認識（faster-whisper）・実姿勢推定/離床検知（MediaPipe）は実装済みだが、要望の分類自体は依然としてキーワードマッチのルールベースで、LLMは使用していない
...
| Phase 4.5 | 巡回・見守りワークフロー（ドメインモデル・状態遷移・ルールベース要望分類・看護師エスカレーション・API・シミュレーション・ダッシュボード・Analytics/Grafana拡張）、実音声認識（faster-whisper）、実姿勢推定/離床検知（MediaPipe）、ドメイン登録・マルチロボット対応、UIリアルタイム更新（pytest 337件）。LLMベースの要望分類・複数病棟対応は未着手 | ✅ |
