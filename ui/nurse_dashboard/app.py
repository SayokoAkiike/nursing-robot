...
# Demo deployment (Streamlit Community Cloud): reused as-is by
# pages/2_看護師ダッシュボード.py, which imports main() and calls it explicitly
# on every rerun -- see ui/patient_request_app/app.py's identical note for
# why (import side effects only run once per process, which would break
# Streamlit's per-rerun model on a second page visit). `streamlit run
# ui/nurse_dashboard/app.py` keeps working exactly as before locally.
